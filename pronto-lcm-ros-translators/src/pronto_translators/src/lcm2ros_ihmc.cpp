...
class LCM2ROS{
  ...
  private:
    ...
    void footstepPlanHandler(const lcm::ReceiveBuffer* rbuf, const std::string &channel, const drc::walking_plan_t* msg);
...
LCM2ROS::LCM2ROS(boost::shared_ptr<lcm::LCM> &lcm_, ros::NodeHandle &nh_): lcm_(lcm_),nh_(nh_) {
  ...
  lcm_->subscribe("WALKING_CONTROLLER_PLAN_RESPONSE",&LCM2ROS::footstepPlanHandler, this);

  pose_body_pub_ = nh_.advertise<nav_msgs::Odometry>("/pose_body",10);
//  walking_plan_pub_ = nh_.advertise<ihmc_msgs::footstepDataLisrt>("/atlas/inputs/ihmc_msgs/FootstepDataListMessage",10);
...
void LCM2ROS::footstepPlanHandler(const lcm::ReceiveBuffer* rbuf, const std::string &channel, const drc::walking_plan_t* msg) {
